    ...
    return (
      <div className="card mb-1 bg-light">
        <div className={`card-header text-primary ${outcomeClass}`}>
          Opp ID: {opportunity.clientSequence} || Department:{" "}
          {departmentString}
        </div>
        <div className="card-body bg-light">
          <h4 className="card-title">{opportunity.oppName}</h4>
          <p className="card-text text-truncate ">{opportunity.description}</p>
          <h6 className="card-title"><u>Close Date:</u></h6>
          <h6 className="card-title">{  (d.getDate() +1)+"/"+
          (d.getMonth()+1)+"/"+    
          d.getFullYear() }</h6>
          <h6 className="card-title"><u>Opportunity Value:</u></h6>
